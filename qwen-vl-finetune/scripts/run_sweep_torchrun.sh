...
# Using torchrun for multi-node distributed training with EFA
#
...
# =============================================================================
MASTER_ADDR="10.1.4.115"  # node0 IP address
NNODES=2
NPROC_PER_NODE=8
NODE1_HOST="node1"  # SSH alias for node1

# Path to torchrun (needed for SSH to node1)
TORCHRUN="/home/ubuntu/.local/bin/torchrun"

# =============================================================================
# EFA environment variables (required for multi-node NCCL communication)
# =============================================================================
export LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libstdc++.so.6
export NCCL_NET_PLUGIN=ofi
export LD_LIBRARY_PATH=/opt/aws-ofi-nccl/lib:/opt/amazon/efa/lib:/opt/amazon/openmpi/lib:/opt/nccl/build/lib:/usr/local/cuda/lib64:$LD_LIBRARY_PATH
export FI_EFA_USE_DEVICE_RDMA=1
export NCCL_DEBUG=WARN

# Environment exports string for SSH commands
ENV_EXPORTS="export LD_PRELOAD=/usr/lib/x86_64-linux-gnu/libstdc++.so.6; \
export NCCL_NET_PLUGIN=ofi; \
export LD_LIBRARY_PATH=/opt/aws-ofi-nccl/lib:/opt/amazon/efa/lib:/opt/amazon/openmpi/lib:/opt/nccl/build/lib:/usr/local/cuda/lib64:\$LD_LIBRARY_PATH; \
export FI_EFA_USE_DEVICE_RDMA=1; \
export NCCL_DEBUG=WARN"
...
SWEEP_LOG_DIR="${LOGS_DIR}/sweep_torchrun_${SWEEP_TIMESTAMP}"
mkdir -p ${SWEEP_LOG_DIR}

echo "=========================================="
echo "Starting training sweep (torchrun) at $(date)"
...
# =============================================================================
# DeepSpeed configuration (still used for ZeRO optimization)
# =============================================================================
...
output_dir_base=/tmp/qwen-vl-finetune/checkpoints/

# =============================================================================
# Configurations to sweep through
...
for config in "${CONFIGS[@]}"; do
    ...
    echo "Launcher: torchrun" >> ${log_file}
    ...
    rm -rf /tmp/qwen-vl-finetune/checkpoints

    # Launch training using torchrun on both nodes
    set +e  # Temporarily disable exit on error to capture failures

    # Launch node1 in background via SSH
    ssh ${NODE1_HOST} "${ENV_EXPORTS}; \
        cd ${PROJECT_DIR}; \
    ...
    # Small delay to let node1 start connecting to rendezvous
    sleep 3

    # Launch node0 (this node)
    ${TORCHRUN} --nnodes=${NNODES} --nproc_per_node=${NPROC_PER_NODE} \
        --node_rank=0 --master_addr=${MASTER_ADDR} --master_port=${MASTER_PORT} \
        ${entry_file} ${args} >> ${log_file} 2>&1 &
    NODE0_PID=$!

    # Wait for node0 to complete
    wait $NODE0_PID
    exit_code=$?

    # Wait for node1 to complete (with timeout)
    ...
    if grep -q "ACCURATE ITERATION TIMING STATISTICS" ${log_file}; then
        ...
        avg_iter_time=$(grep "Average iteration time (excluding warmup):" ${log_file} | tail -1 | grep -oP '\K[0-9.]+(?= seconds)' || echo "N/A")

        # Extract steps per second
        train_steps_per_sec=$(grep "Steps per second:" ${log_file} | tail -1 | grep -oP '\K[0-9.]+$' || echo "N/A")

        # Extract min and max times
        min_iter_time=$(grep "Min iteration time:" ${log_file} | tail -1 | grep -oP '\K[0-9.]+(?= seconds)' || echo "N/A")
        max_iter_time=$(grep "Max iteration time:" ${log_file} | tail -1 | grep -oP '\K[0-9.]+(?= seconds)' || echo "N/A")
    fi
    ...
    if [ $exit_code -eq 0 ]; then
        ...
        # Kill any remaining processes on node1
        ssh ${NODE1_HOST} "pkill -f 'torchrun.*${MASTER_PORT}'" 2>/dev/null || true
...
echo "Training Sweep Summary (torchrun)" > ${summary_file}
echo "==================================" >> ${summary_file}
echo "Started: ${SWEEP_TIMESTAMP}" >> ${summary_file}
echo "Completed: $(date)" >> ${summary_file}
echo "Launcher: torchrun" >> ${summary_file}
echo "Nodes: ${NNODES}" >> ${summary_file}
echo "GPUs per node: ${NPROC_PER_NODE}" >> ${summary_file}
echo "" >> ${summary_file}
